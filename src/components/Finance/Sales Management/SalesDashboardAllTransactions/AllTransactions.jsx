...
const AllTransactions = () => {
  ...
  function getData() {
    axios
      .get(baseUrl + "/sales/payment_update", {
        headers: {
      ...
      .then((res) => {
        ...
        const sortedStatusData = custData?.sort(
          (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
        );
        setData(sortedStatusData);
        setFilterData(sortedStatusData);

        const uniqueCustomers = new Set(
          sortedStatusData.map((item) => item.account_name)
        );
        setUniqueCustomerCount(uniqueCustomers.size);
        const uniqueCustomerData = Array.from(uniqueCustomers).map(
          (customerName) => {
            return sortedStatusData.find(
        ...
        const dateFilterData = filterDataBasedOnSelection(sortedStatusData);
          ...
          <div className="card">
            ...
            <div className="card-body">
              ...
              <h4 className="font-weight-bold mt8">
                ...
                {datas.length > 0
                  ? datas
                      .filter((item) => item.payment_approval_status == "approval")
                      .reduce((total, currentItem) => {
                  ...
                  <div className="form-group">
                    ...
                    <Autocomplete
                      ...
                      options={Array.from(
                        new Set(
                          datas?.map(
                            (option) => option?.sales_executive_name || []
                          )
                        )
                      )}
                      renderInput={(params) => (
