...
export const downloadExcel = async ({
  ...
  console.log("renamedCategories----", renamedCategories);
  var serialNumber = 1;
    ...
    if (platform === "Instagram") {
      platformData.forEach((page) => {
        if (page.platform_name === "instagram") {
          // const categoryId = page.page_sub_category_id;
          const pageLink = `https://www.${platform.toLowerCase()}.com/${
            page.page_name
          }`;
          // const categoryName = category?.find((cat) => cat._id === categoryId)?.page_sub_category || 'Unknown';
          const catName = page.page_sub_category_name || "Unknown";
          let subCategoryId = null;
          if (Object.keys(renamedCategories).length > 0) {
            const matched = subCategory.find(
              (cat) => cat.page_sub_category.toLowerCase() === catName.toLowerCase()
            );
            subCategoryId = matched?._id;
          }
          // renamedCategories?.[categoryId] ||
          const categoryName = renamedCategories?.[subCategoryId]||page.page_sub_category_name || "Unknown";
          // console.log("categorrryId------", categoryId);
  ...
  if (twitterTrendCost > 0) {
    let twitterTrendRow = overviewSheet.addRow([
      "",
      "Twitter Trend Cost",
      "",
      "",
      "",
      "",
      `₹${formatIndianNumber(twitterTrendCost)}`,
    ]);
  ...
  // UGC Cost
  if (ugcVideoCost > 0) {
    let ugcRow = overviewSheet.addRow([
      "",
      "UGC Cost",
      "",
      "",
      "",
      "",
      `₹${formatIndianNumber(ugcVideoCost)}`,
    ]);
  ...
  let totalWithGstRow = overviewSheet.addRow([
    ...
    "",
    `₹${formatIndianNumber(
      totalCostWithGst + ugcVideoCost + twitterTrendCost
    )}`,
  ]);
  ...
  const twitterAndUgc =
    twitterTrendCost > 0 && ugcVideoCost > 0
      ? 5
      : twitterTrendCost > 0 || ugcVideoCost > 0
      ? 4
      : 3;
