...
const IncentiveRequest = () => {
  ...
  async function fetchData() {
    ...
    try {
      ...
      response = await axios.get(
        baseUrl + `sales/incentive_request_user_status_wise/${userData?.id}`,
        {
          headers: {
  ...
  const columns = [
    ...
    },
    {
      key: "createdAt",
      name: "Requested Date",
      renderRowCell: (row) => <div>{DateISOtoNormal(row.createdAt)}</div>,
      width: "150",
    },
  ];
    ...
    columns.push(
      {
        key: "payment_date",
        name: "Payment Date",
        renderRowCell: (row) => <div>{DateISOtoNormal(row.payment_date)}</div>,
        width: "150",
      },
  ...
  return (
    <div>
      ...
      <View
        ...
        title={
          userData?.type === "released"
            ? "Incentive Request Overview"
            : "Incentive Release Overview"
        }
        tableName={
          userData?.type === "released"
            ? "sales_incentive_request_Dashboard"
            : "sales_incentive_released_Dashboard"
        }
      />
    </div>
  );
