...
const EarnedAndUnearned = () => {
    ...
    const [isEarnedAndUnearnedDataLoading, setIsEarnedAndUnearnedDataLoading] = useState(false);
    ...
    async function getEarnedAndUnearnedData() {
        ...
        try {
            const response = await axios.get(`${baseUrl}sales/incentive_calculation_status_wise_data/${userData.id}?incentive_earning_status=${userData.status}${userData.flag === 0 ? `&year=${extractYear(userData.month)}&month=${extractMonthNumber(userData.month)}` : ''}`, {
                headers: {
    ...
    const columns = [{
        ...
        width: 100,
        renderRowCell: (row) => row.incentive_earning_status === "earned" ? "Earned" : "Unearned"
    }, {
