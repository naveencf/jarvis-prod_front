...
import { useDeletePageCategoryMutation, useGetAllPageCategoryQuery } from '../../../Store/PageBaseURL';
import { IconButton, Stack } from '@mui/material';
import { setModalType, setOpenShowAddModal, setRowData } from '../../../Store/PageMaster';
...
const CategoryOverview = () => {
  ...
  const [deletePageCategory, { data: deletedResponse, error: deleteError }] = useDeletePageCategoryMutation();
...

    {
      ...
      renderRowCell: (row) => (
        <div className="flex-row">
          <button title="Edit" className="icon-1" onClick={() => handlRowClick(row, 'Category Update')} data-toggle="modal" data-target="#myModal">
            <FaEdit />{' '}
          </button>
      ...
      <div className="action_heading">
        ...
        <div className="action_btns">
          ...
          <Link to="/admin/pms-page-sub-category">
            <button className="btn cmnbtn btn-primary btn_sm">Sub Category</button>
          </Link>
        </div>
      </div>
      <View title={''} data={categoryData} columns={categoryGrid} pagination tableName={''} />
    </div>
  );
};
