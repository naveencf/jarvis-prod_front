          ...
          <Table aria-label="vendor-details-table">
            ...
            <TableBody>
              {Object.entries({
                ...
                "Vendor Type": typeData?.find(
                  (item) => item?._id == previewData?.vendor_type
                )?.type_name,
                "Vendor Platform": platformData?.find(
                  (item) => item?._id == previewData.vendor_platform
                )?.platform_name,
                "Pay Cycle": cycleData?.find(
                  (item) => item?._id == previewData?.pay_cycle
                )?.cycle_name,
                ...
                <Grid container spacing={2}>
                  {row.payment_method === "666856874366007df1dfacde" && (
                    <>
                      ...
                      <Grid item xs={6}>
                        <Typography>
                          <strong>IFSC Code:</strong> {row?.ifsc || "N/A"}
                        </Typography>
                      </Grid>
                      <Grid item xs={6}>
                        <Typography>
                          <strong>Pan No:</strong> {row?.pan_card || "N/A"}
                        </Typography>
                      </Grid>
                      <Grid item xs={6}>
                        <Typography>
                          <strong>Account Holder Name:</strong>{" "}
                          {row?.account_holder_name || "N/A"}
                        </Typography>
                      </Grid>
                    </>
                  )}
          ...
          <TableContainer component={Paper} sx={{ my: 1 }} key={index}>
            <Table aria-label="document-details-table">
              <TableBody>
                <TableRow>
                  <TableCell>
                    <strong>Document Name:</strong> {doc?.docName || "N/A"}
                  </TableCell>
                  <TableCell>
                    <strong>Document Number:</strong> {doc?.docNumber || "N/A"}
                  </TableCell>
                </TableRow>
              </TableBody>
            </Table>
