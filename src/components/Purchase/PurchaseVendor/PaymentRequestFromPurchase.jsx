...
const PaymentRequestFromPurchase = ({ reqestPaymentDialog, setReqestPaymentDialog, vendorDetail, setVendorDetail, userName }) => {
    ...
    useEffect(() => {
        axios.post(phpBaseUrl + `?view=getvendorDataListvid`, {
            "vendor_id": vendorDetail?.vendor_id
        }).then((res) => {
            if (res.status == 200) {
        ...
        axios.get(`${baseUrl}` + `v1/bank_details_by_vendor_id/${vendorDetail?.vendor_id}?isNumberId=true`, {
            headers: {
    ...
    return (
        ...
        >
            ...
            <DialogContent>
                ...
                {vendorBankDetail != "" && <FormControl fullWidth sx={{ maxWidth: 360 }}>
