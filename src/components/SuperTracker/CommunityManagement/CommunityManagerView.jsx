  ...
  const fetchRows = async () => {
    try {
      ...
      const countMap = data.reduce((acc, item) => {
        ...
        acc[reportTo].followerGrowthCount +=
          item.reportStatus?.previousDay.todayVsYesterdayFollowersCountDiff ||
          0;
  ...
  const columns = [
    {
      ...
      headerName: "S.NO",
      valueGetter: (params) =>
        rows.findIndex((row) => row.reportTo === params.row.reportTo),
      renderCell: (params) => {
        const rowIndex = rows.findIndex(
          (row) => row.reportTo === params.row.reportTo
        );
    ...
    <>
      <div className="action_heading">
        <div className="form-heading">
          <h2 className="mb0 mt-1">Manager View</h2>
        </div>
              ...
              <TableBody>
                {modalData?.map((page, index) => (
                  ...
                  >
                    ...
                    <TableCell align="left">{page._id}</TableCell>
                    <TableCell>
                      {page.creatorInfo?.followersCount || "N/A"}
                    </TableCell>
                    <TableCell>
                      <Button variant="outlined" color="error">
                        {page.reportStatus?.previousDay
                          ?.todayVsYesterdayFollowersCountDiff || "N/A"}
                      </Button>
                    </TableCell>
                    <TableCell>
