...
const CommunityUser = () => {
    ...
    useEffect(() => {
        const initialCategories = category
            .filter(option => assignedCategoryData?.pageCategoryIds?.includes(option.category_id))
            .map(option => ({ value: option.category_id, label: option.category_name }));

        setSelectedCategories(initialCategories);
    }, [category, assignedCategoryData]);

    const handleChange = (selectedOptions) => {
        const updatedCategories = selectedOptions?.map(option => option.value) || [];
    ...
    const handleOpenAssignedCategory = async (row) => {
        const res = await axios.get(`https://insights.ist:8080/api/v1/community/category_manager_by_user/${row.user_id}`)
        setAssignedCategoryData(res.data.data);
    ...
    const handleAssignCategory = async (e) => {
        ...
        try {
            const response = await axios.post('https://insights.ist:8080/api/v1/community/category_manager', {
            ...
            alert('Category assignment failed. It may already be assigned, or an error occurred. Please try again.');
        }
    };
    ...
    const
        handleUpdate = async (catId) => {
            try {
                const updatedCategoryIds = assignedCategoryData?.pageCategoryIds.filter(item => item !== catId);
                const updatedResponse = await axios.put('https://insights.ist:8080/api/v1/community/category_manager', {
        ...
        <>
            ...
            <Box sx={{ height: 500, width: '100%' }}>
                <Button
                    ...
                    color="error"
                    sx={{ float: "right", mb: 1 }}
                    onClick={() => navigate('/admin/instaapi/community/allAssignedcategory')}
                >
            ...
            >
                <Button variant="outlined" sx={{ float: 'right' }} onClick={handleCloseAssignCategory} color='error'> X </Button>
                ...
                <div className="card">
                    ...
                    <div className="card-body">
                        <div className="row">
                            <FieldContainer
                                ...
                                fieldGrid={12}
                                disabled
                                value={userContextData?.find(user => user.user_id === rowData?.user_id)?.user_name || "N/A"}
                            />
                    ...
                    <div className="w-100 sb">
                        <div></div>
                        <button className='btn btn-primary cmnbtn mb-2 mr-3' onClick={handleAssignCategory}>Submit</button>
                    </div>
                </div>
            </Modal>
            ...
            >
                <Button variant="outlined" sx={{ float: 'right' }} onClick={handleCloseAssignedCategory} color='error'> X </Button>
                <Button variant="outlined" sx={{ float: 'right' }} onClick={toggleUpdateCategory} color='error'> Update Category </Button>
                ...
                {showUpdateCategory ? (
                    <div className="update-category-section">
                        ...
                        <form>
                            ...
                            <Button variant="contained" color="primary" onClick={handleUpdateRowWiseCategory}>Save</Button>
                        </form>
                    </div>
                ) :
                    <div className="card">
                        ...
                        <div className="card-body">
                            <table className="table table-striped">
                                ...
                                <tbody>
                                    {assignedCategoryData?.pageCategoryIds?.length > 0 ? (assignedCategoryData?.pageCategoryIds?.map((itemsData, index) => {
                                        const categoryObj = category?.find(cat => cat?.category_id === itemsData);
                                        return (
                                            <tr key={index}>
                                                <th scope="row">{index + 1}</th>
                                                <td>{categoryObj ? categoryObj?.category_name : 'Unknown Category'}</td>
                                                <td><button className='btn btn-primary cmnbtn' onClick={() => handleUpdate(categoryObj?.category_id)}>UnAssigned</button></td>
                                            </tr>
                                        );
